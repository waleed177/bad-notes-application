...
func setup(data: TabItemModel, setup_as_draggable=false):
	...
	$Button.text = data.tab_model.text
	data.tab_model.connect("property_modified", self, "_on_data_property_modified")
...
func _gui_input(event):
	if event is InputEventMouseButton:
		if event.pressed and event.button_index == BUTTON_RIGHT:
			...
			$ConfirmationDialog/LineEdit.text = _data.tab_model.text
...
func _on_button_pressed():
	...
	get_tree().current_scene.use_drawing_area(_data.tab_model.drawing_area.drawing_area_node)
...
func _on_instantiated_item(item):
	var new_data = _data.duplicate()
	item.setup(new_data)
	NoteSaver.register_item(new_data)

func _on_ConfirmationDialog_confirmed():
	_data.tab_model.text = $ConfirmationDialog/LineEdit.text
